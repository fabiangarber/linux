...
void *irq_wait_thread(void *arg) {
    ...
    if (debugMode) {
        printf("IRQ wait thread exiting with gpio_change_count = %d\n", gpio_change_count);
        fflush(stdout); // Flush the output buffer
    }
    return NULL;
}
...
void *sim_thread(void *arg) {
    ...
    while (1) {
        // usleep(1000);
        data.pin = rand() % 8;
        if (ioctl(fd, GPIO_GET_VALUE, &data) < 0) {
            perror("Error getting GPIO value");
        } else {
            if (debugMode) {
                printf("Read GPIO value: pin %d is %d\n", data.pin, data.value);
                fflush(stdout); // Flush the output buffer
            }
        }
        // Sleep for a short duration to simulate real-world delays
        usleep(100000); // Sleep for 100 milliseconds
    }
    if (debugMode) {
        printf("Simulation thread exiting with gpio_change_count = %d\n", gpio_change_count);
        fflush(stdout); // Flush the output buffer
    }
    return NULL;
}
