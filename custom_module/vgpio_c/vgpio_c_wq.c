...
static atomic_t gpio_changed = ATOMIC_INIT(0);
...
{
    if (wait_event_interruptible(gpio_wait_queue, atomic_read(&gpio_changed)))
        return -ERESTARTSYS; // If interrupted

    atomic_set(&gpio_changed, 0);
    char data = '1';
